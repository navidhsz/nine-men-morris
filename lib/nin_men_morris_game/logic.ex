defmodule NineMenMorrisGame.Logic do
  @moduledoc false
  ...
  def can_move(
    ...
    case NineMenMorrisGame.MovementRules.valid_selection?(player_name, from_pos, from_pos_state) do
      :ok ->
        case NineMenMorrisGame.MovementRules.valid_destination?(
               player_name,
               to_pos,
               to_pos_state
             ) do
          :ok ->
            if NineMenMorrisGame.MovementRules.move_possible?(from_pos, to_pos) do
              :ok
            else
  ...
  def mill?(board_name, player_name, pos) do
    mill_scenarios = NineMenMorrisGame.Definitions.get_possible_mill_scenario(pos)
    NineMenMorrisGame.State.get_board(board_name) |> _mill?(player_name, mill_scenarios)
  end
